...
user_name_pass =  ((conf['username'] if 'username' in conf else '') + 
                  (':' + conf['password'] if 'password' in conf else '') + '@') 
user_name_pass = '' if user_name_pass == '@' else user_name_pass

broker = (conf['type'] + '://' + user_name_pass + 
         conf['host'] + (':' + str(conf['port']) if 'port' in conf else '') + '/' + (str(conf['instance']) if 'instance' in conf else ''))

app = Celery('job_runs', broker=broker, backend=broker + '?' + (conf['arguments'] if 'arguments' in conf else ''))

...
class TestRule(BaseTest):
    ...
    @dummy_rule
    def test_all_tables_with_source_returns_proper_source(self, r, d):
        job_template = JobTemplate(data_sources=[d])
        result = r.all_tables_with_source(JobRun(job_template=job_template))
        self.assertEqual(d, result[0][0])


    @dummy_rule
    def test_all_tables_with_source_returns_proper_tables(self, r, d):
        job_template = JobTemplate(data_sources=[d])
        result = r.all_tables_with_source(JobRun(job_template=job_template))
    ...
    def run_rule(self, r, d):
        checks_to_run = []
        job_template = JobTemplate(data_sources=[d])
        checks = r.run(JobRun(job_template=job_template), checks_to_run)
        tables = r.all_tables_with_source(JobRun(job_template=job_template))[0][1]
        return checks_to_run


    @dummy_rule
    def test_rule_runs_returns_proper_check(self, r, d):
        checks_to_run = self.run_rule(r,d)
        self.assertTrue((d, 'test.test_uniqueness_fail', r.checks[0]) in checks_to_run)


    @dummy_rule
    def test_rule_runs_returns_proper_number_of_checks(self, r, d):
        checks_to_run = self.run_rule(r,d)
    ...
    @dummy_rule
    def test_rule_runs_doesnt_return_improper_checks(self, r, d):
        checks_to_run = self.run_rule(r,d)
        # Only tables with id present(the rule in dummy rule) are the uniqueness fail/succ tables and null check tables.
    ...
    @dummy_rule
    def test_rule_runs_include_children_rule_checks(self, r, d):
